...
function init(){
    ...
    bindEvents();
    fullScreen = BrowserWindow.getFocusedWindow().isFullScreen();

    document.querySelector('#videoContainer').ondblclick = function(){

        // Saving to variable here because checking the window for the state took to long.
        if(fullScreen == true) {
    ...
    var showDialogButton = document.querySelector('#settings-dialog');
    if (! dialog.showModal) {
    ...
    */
    ...
    window.onbeforeunload = (e) => {
      ...
      storage.set("window", {
       ...
       maximized: BrowserWindow.getFocusedWindow().isMaximized()
